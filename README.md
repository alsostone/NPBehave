## 为什么使用NPBehave
- 轻量，快速，简洁，用代码构建行为树。
- 事件驱动，不需要每帧从根节点遍历。它们保持当前状态，只有在实际需要时才继续遍历。这使得它们的性能更高。
- 与引擎藕合较低，意味着剥离成本低。

## 要解决的痛点

- 由于要用到帧同步战斗中，有纯逻辑需求`引擎无关性`，需移除运行时对Unity的依赖。
- 需使用MemoryPack做序列化/反序列化，故回调逻辑需改用其他方式。
  - 使用回调的节点改为使用继承，同时继承方式对后续做编辑器也是有益的。
  - 
- 可视化编辑器`项目初期，行为树由程序构建，有没有编辑器影响不大，往后放一下`
- 调试`自带的Debugger能查看行为树的运行状态，暂时够用`

## 原始链接
[https://github.com/meniku/NPBehave](https://github.com/meniku/NPBehave)

## 正文
如果没有以上痛点，请直接使用原始工程。以下同步更新原工程的ReadMe。`提交记录可作为参考`。
...
```
当您运行此命令时，您将注意到“Hello World”将被一次又一次地打印出来。这是因为当遍历过树中的最后一个节点时，根节点将重新启动整个树。如果不需要这样，可以添加一个WaitUntilStopped节点，如下所示:
```csharp
// ...
...
#### WaitSecond
- WaitSecond(float seconds): 等待给定的秒，随机误差为0.05 *秒
- WaitSecond(float seconds, float randomVariance): 用给定的随机变量等待给定的秒数

#### WaitBlackboardKey
- WaitBlackboardKey(string blackboardKey, float randomVariance = 0f): 使用黑板中指定键的值作为等待的秒数

#### WaitCalc
- WaitCalc(float randomVariance = 0f): 需要在子类中实现CalcSeconds 使用其返回的秒数来等待
	
#### WaitUntilStopped
- WaitUntilStopped(bool successWhenStopped = false):等待被其他节点停止。它通常用于Selector的末尾，等待任何before头的同级BlackboardCondition、BlackboardQuery或Condition变为活动状态。
